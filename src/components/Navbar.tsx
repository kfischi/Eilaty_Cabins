      ...
      >
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <a href="#" style={{ textDecoration: "none", display: "flex", flexDirection: "column", lineHeight: 1 }}>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.45rem", fontWeight: 600, color: "#e8c547", letterSpacing: "1px" }}>
              בקתות צוריאל
            </span>
          ...
          <div className="desktop-nav" style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <a href="#booking" style={{ background: "linear-gradient(135deg, #c9a227, #e8c547)", color: "#0a0f0a", padding: "11px 26px", borderRadius: 50, fontSize: "0.88rem", fontWeight: 700, textDecoration: "none", letterSpacing: "0.3px", transition: "all 0.3s", boxShadow: "0 4px 20px rgba(201,162,39,0.3)" }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 32px rgba(201,162,39,0.5)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(201,162,39,0.3)"; }}
            >
              בדקו זמינות
            </a>
          </div>
          ...
          <div className="mobile-nav" style={{ display: "none", alignItems: "center", gap: 10 }}>
            ...
            <motion.button onClick={() => setMenuOpen((p) => !p)} whileTap={{ scale: 0.92 }}
              style={{ width: 44, height: 44, background: "rgba(201,162,39,0.12)", border: "1px solid rgba(201,162,39,0.3)", borderRadius: 10, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5 }}
              aria-label="תפריט"
            >
              {[0, 1, 2].map((i) => (
                <motion.span key={i}
                  animate={i === 0 ? { rotate: menuOpen ? 45 : 0, y: menuOpen ? 9 : 0 } : i === 1 ? { opacity: menuOpen ? 0 : 1 } : { rotate: menuOpen ? -45 : 0, y: menuOpen ? -9 : 0 }}
                  style={{ display: "block", width: i === 1 ? 16 : 22, height: 1.5, background: "#e8c547", borderRadius: 2, transformOrigin: "center" }}
                />
              ))}
            </motion.button>
          </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <div style={{ position: "absolute", top: 0, left: "10%", right: "10%", height: "1px", background: "linear-gradient(90deg, transparent, #e8c547, transparent)" }} />

            <div style={{ textAlign: "center", marginBottom: 48 }}>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "2rem", fontWeight: 600, color: "#e8c547" }}>בקתות צוריאל</div>
            ...
            <div style={{ flex: 1, padding: "0 40px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 4 }}>
              {navLinks.map((item, i) => (
                <motion.button key={item.href}
                  ...
                  style={{ background: "none", border: "none", borderBottom: "1px solid rgba(201,162,39,0.08)", color: "#f7f2ea", fontFamily: "'Cormorant Garamond', serif", fontSize: "2.2rem", fontWeight: 500, padding: "20px 0", cursor: "pointer", textAlign: "right", display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", letterSpacing: "0.5px" }}
                  onMouseEnter={(e) => (e.currentTarget.style.color = "#e8c547")}
                  onMouseLeave={(e) => (e.currentTarget.style.color = "#f7f2ea")}
                >
                  {item.label}
                  <span style={{ fontSize: "1rem", color: "rgba(201,162,39,0.4)", fontFamily: "Heebo" }}>←</span>
                </motion.button>
              ))}
            </div>
            ...
            >
              <a href="#booking" onClick={() => go("#booking")}
                style={{ display: "block", textAlign: "center", background: "linear-gradient(135deg, #c9a227, #e8c547)", color: "#0a0f0a", padding: "18px", borderRadius: 14, fontWeight: 800, fontSize: "1.05rem", textDecoration: "none" }}>
                בדקו זמינות עכשיו ✦
              </a>
